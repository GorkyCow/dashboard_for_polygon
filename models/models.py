...
class StocksAggregates(Base):
    ...
    def __repr__(self):
        return (f"<StocksAggregates(ticker='{self.ticker}', timestamp='{self.timestamp}', volume={self.volume}, " +
    ...
    def get_stock_aggregates(ticker, from_, to):
        ...
        for aggregate in session.query(StocksAggregates).\
    ...
    def save_stock_aggregates(ticker, resp_aggregates):
        session = Session()
        stock_aggregates = [StocksAggregates(ticker=ticker, timestamp=ts_to_datetime(aggregate["t"]), volume=aggregate["v"], volume_weight=aggregate["vw"], open=aggregate["o"], close=aggregate["c"], highest=aggregate["h"], lowest=aggregate["l"], number=aggregate["n"]) for aggregate in resp_aggregates]
    ...
    def get_first_day(ticker):
        ...
        result = session.query(StocksAggregates).\
                         filter_by(ticker=ticker).\
                         order_by(asc(StocksAggregates.timestamp)).\
                         first()  
        session.close()
...
class StocksNames(Base):
    ...
    def __repr__(self):
        return f"<StocksNames(id='{self.id}', ticker='{self.ticker}', name={self.name})>"
    ...
    def get_stock_names():
        ...
        for aggregate in session.query(StocksNames).\
                        order_by(asc(StocksNames.ticker)):
            result.append(aggregate)
        session.close()
        return result
    ...
    def get_first_ticker():
        ...
        result = session.query(StocksNames).\
                         order_by(asc(StocksNames.ticker)).\
                         first()  
        session.close()
