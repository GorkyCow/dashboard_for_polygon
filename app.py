...
app.layout = html.Div(style={'backgroundColor': colors['background']}, children=[
    ...
    dcc.Dropdown(
        id="paper",
        options=[{"label": name.name, "value": name.ticker} for name in md.StocksNames.get_stock_names()],
        value=md.StocksNames.get_first_ticker()
    ),
    html.Label('Day:'),
    dcc.Dropdown(
        id="date",
        options=[{"label": date, "value": date} for date in md.StocksAggregates.get_days("AAPL")],
