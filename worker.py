...
from utils import get_last_n_days, get_next_day_str
    ...
    with RESTClient(key) as client:
        try:
            resp = client.stocks_equities_aggregates(
                ticker.ticker, 1, "minute", from_, to, unadjusted=True
            )
        ...
        if hasattr(resp, "results"):
            new_added = md.StocksAggregates.save_stock_aggregates(
                resp.ticker, resp.results
            )
...
if __name__ == "__main__":
    ...
    days_to_collect = get_last_n_days(
        date.today().strftime("%Y-%m-%d"), n=config.DAYS_TO_LOAD
    )
